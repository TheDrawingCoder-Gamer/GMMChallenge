...
// Trait for FUTURE :heart_eyes:
pub trait ModItem: Clone {
...
pub fn all_deps_of_selected<M: ModItem>(selected: Vec<M>) -> Vec<String> {
    let mut deps: Vec<String> = selected
        .iter()
    ...
    let good_deps = deps
        .iter()
        .map(|it| selected.iter().find(|ti| ti.identifier() == it.clone()))
        .flatten()
        .map(|it| it.clone())
        .collect_vec();
    let mut their_deps = all_deps_of_selected(good_deps);
    deps.append(&mut their_deps);
    deps.sort_unstable();
    deps.dedup();
    deps
}
pub fn handle_dependencies<M: ModItem>(items: &mut Vec<M>)  -> (){
    // Don't include disabled
    let selected: Vec<M> = items.iter().filter(|it| it.selected() && !it.disabled()).map(|it| it.clone()).collect();
    let deps = all_deps_of_selected(selected);
    for dep in deps {
        match items.iter_mut().find(|it| it.identifier() == dep) {
            Some(it) => {
